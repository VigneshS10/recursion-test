...
The key output should be a compact success-rate-versus-log-slope table. Also report the fraction of the full `alpha=0.01` rescue already achieved at `1e-4` and `1e-3` so the near-binary versus dose-dependent hypotheses can be distinguished directly.
...
- `handoff.json` with run ID `research_20260912_180011_79ef`, copied task nonce, a **fresh evidence nonce not reused from any earlier run**, status, and artifact references.
