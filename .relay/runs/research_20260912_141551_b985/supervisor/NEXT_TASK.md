...
- Exactly 200 updates.
- At least 200 paired seeds, preferably seeds `0..199`.
...
The important control is pairing: for a given initialization scheme and seed, both activations must start from exactly the same weights and biases.

Do not tune learning rate, width, optimizer, or training budget separately by activation or initialization after seeing results.
...
Also classify the final state of failed runs, at minimum distinguishing 50%, 75%, and 100% accuracy. Preserve lightweight loss trajectories for a small fixed subset of representative seeds rather than storing every step for all runs if file size becomes unnecessary.

## Controls
- Same XOR dataset and target encoding.
- Same architecture and optimizer.
- Same exact 200-step budget.
- Same seed set for every condition.
- ReLU and `tanh` paired from identical initial parameters within each seed and initialization scheme.
- Initialization scheme is the only additional independent variable beyond activation.
...
Also report the activation success-rate difference (`tanh - ReLU`) separately under He and Xavier initialization. Include a simple paired statistical comparison appropriate for binary outcomes, such as an exact McNemar test or an exact/binomial analysis of the discordant pairs. Keep the statistical analysis small and transparent.

A compact table should make it possible to answer whether the activation effect is stable across initialization schemes and whether the previous 20-seed result replicates at larger scale.

## Completion criteria
The task is complete when all seeds have been run for all four conditions and the evidence supports one of these conclusions:
...
Do not add new activations, widths, optimizers, or budgets in this run. This experiment should resolve the initialization ambiguity in the previous result before broadening the study.
...
- `handoff.json` with the next run ID, copied task nonce, fresh evidence nonce, status, and artifact references.
